...
const App: React.FC = () => {
            ...
            {Object.values(GameMode).map((mode) => {
              ...
              return (
                <button
                  ...
                  className="relative group h-80 w-full rounded-3xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 focus:outline-none focus:ring-4 focus:ring-rose-200"
                >
                   {/* Background Gradient */}
                   ...
                   <div className="relative h-full flex flex-col items-center justify-center p-8 text-white">
                      <div className="w-24 h-24 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center text-5xl mb-6 shadow-inner transition-transform group-hover:scale-110 duration-500">
                        {config.emoji}
                      </div>
                      <h3 className="text-3xl font-bold mb-2 tracking-wide">{config.label}</h3>
                      <p className="text-white/90 font-medium text-center">{config.description}</p>
                      
                      <div className="absolute bottom-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300 translate-y-2 group-hover:translate-y-0">
  ...
  // --- Game Screen (Desktop) ---
  return (
    <div className="h-screen w-screen bg-slate-100 flex flex-col relative overflow-hidden">
      {/* Dynamic Background */}
      ...
      <main className="flex-1 flex items-center justify-center w-full h-full p-8 gap-16">
        
        {/* Left Side: Controls/Status (Visible on Desktop) */}
        ...
        <div className="flex flex-col items-center">
           ...
           {/* Action Bar (Mobile/Tablet or Fallback) */}
           <div className={`mt-10 transition-all duration-500 transform ${isFlipped && !loading ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
            <button
              onClick={handleNextCard}
              className={`
                flex items-center gap-2 px-10 py-4 rounded-full shadow-xl shadow-rose-200/50 
                bg-gradient-to-r ${MODE_CONFIG[currentMode].gradient} text-white font-bold text-xl
           ...
           {!isFlipped && (
             <p className="text-slate-400 text-sm animate-bounce mt-8 font-medium">点击卡片或按空格键翻开</p>
           )}
        </div>
      </main>
    </div>
